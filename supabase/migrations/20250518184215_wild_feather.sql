/*
  # Fix ambiguous column references in get_admin_stats function

  1. Changes
    - Fix ambiguous creator_id references by specifying table names
    - Add proper date filtering
    - Improve query performance with explicit joins
*/
...
BEGIN
  ...
  SELECT json_build_object(
    'totalUsers', (SELECT COUNT(*) FROM users WHERE users.id = creator_id),
    'totalProducts', (
      SELECT COUNT(*) 
      FROM products 
      WHERE products.creatorid = creator_id
    ),
    'totalOrders', (
      SELECT COUNT(*) 
      FROM orders o
