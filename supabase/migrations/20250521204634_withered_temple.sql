/*
  # Fix RLS policies for product file uploads

  1. Changes
    - Create maximally permissive storage policies without folder path restrictions
    - Ensure product-files bucket is properly configured
    - Add owner_uid column to products table for proper RLS
    - Fix public access settings for buckets
...
CREATE POLICY "product_images_select_20250522"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'product-images');

CREATE POLICY "product_images_insert_20250522"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (bucket_id = 'product-images');

CREATE POLICY "product_images_update_20250522"
ON storage.objects FOR UPDATE
TO authenticated
USING (bucket_id = 'product-images')
WITH CHECK (bucket_id = 'product-images');

CREATE POLICY "product_images_delete_20250522"
ON storage.objects FOR DELETE
TO authenticated
USING (bucket_id = 'product-images');

-- Create maximally permissive policies for product-files bucket
CREATE POLICY "product_files_select_20250522"
ON storage.objects FOR SELECT
TO authenticated
USING (bucket_id = 'product-files');

CREATE POLICY "product_files_insert_20250522"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (bucket_id = 'product-files');

CREATE POLICY "product_files_update_20250522"
ON storage.objects FOR UPDATE
TO authenticated
USING (bucket_id = 'product-files')
WITH CHECK (bucket_id = 'product-files');

CREATE POLICY "product_files_delete_20250522"
ON storage.objects FOR DELETE
TO authenticated
USING (bucket_id = 'product-files');
